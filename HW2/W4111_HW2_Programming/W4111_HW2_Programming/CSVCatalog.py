...
class IndexDefinition:
    ...
    def add_column_to_index(self, col_name):
...
class TableDefinition:
    ...
    def load_columns(self):
        ...
        for res in result:
            new_c = ColumnDefinition(column_name=res['column_name'],
                                     column_type= res['type'],
                                     not_null = (res['not_null']==1))

            self.columns.append(new_c)
    def load_indexes(self):
        ...
        result = run_q(self.cnx, q, args=self.table_name, fetch=True)

        self.indexes = {}

        # temp_cols = defauldict(list)
        # temp_type = {}
        index_defs = {}
        if result is not None:
            for row in result:
                index = index_defs.get(row['index_name'], None)
                if index is None:
                    index = IndexDefinition(row['index_name'], row['type'], [row['column_name']])
                    index_defs[row['index_name']] = index
                else:
                    index_defs[row['index_name']].add_column_to_index(row['column_name'])

        self.indexes= index_defs
    ...
    def drop_col_in_sql(self, cn):
        ...
        run_q(self.cnx, q, cn, fetch=False)
    ...
    def to_json(self):
        ...
        if self.indexes is None:
            ...
            for idx, v in self.indexes.items():
    ...
    def define_index(self, index_name, columns, type="index"):
        ...
        index = self.indexes.get(index_name, None)
    ...
    def get_index(self, ind_name):
        ...
        if self.indexes is None:
            ...
            index = self.indexes.get(ind_name, None)
...
class CSVCatalog:
    ...
    def __init__(self, dbhost="localhost", dbport=3306,
                 dbuser="root", dbpw="admin123", db="CSVCatalog", debug_mode=None):
